            ...
            <div className="grid gap-6 md:grid-cols-1">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription className="text-center">Current account credit</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="text-center">
                            <p className="text-4xl font-bold text-green-600">{user?.userAccount?.amount} so'm</p>
                            ...
                            <div key={transaction?.id} className="flex items-center justify-between border-b pb-4">
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-sm font-medium">{transaction?.aiBot?transaction?.aiBot?.name?.uz:"Payment"}</span>
                                    </div>
                                    <p className="text-xs text-muted-foreground">{new Date(transaction.lastPaymentDate??transaction?.createdAt).toLocaleString()}</p>
                                </div>
                                <div className="text-right">
                                    <p className={`text-sm font-medium ${
                                        !transaction?.aiBot ? 'text-green-600' : 'text-red-600'
                                    }`}>
                                        {transaction?.aiBot?"-":"+"}{Math.abs(transaction.amount).toFixed(2)} so'm
                                    </p>
                                </div>
                            </div>
                        ))}
